...
// если пользователь является консультантом, получаем список пользователей
if ($role === 'consultant') {
...
// если пользователь - обычный пользователь, проверяем наличие сообщений от консультанта
if ($role === 'user') {
    ...
    // если консультант не найден, сохраняем сообщение
    if ($consultant) {
...
// если получен ID получателя (консультанта), загружаем сообщения
if (isset($_GET['receiver_id']) || isset($consultant_id)) {
    $receiver_id = ($role === 'consultant') ? $_GET['receiver_id'] : $consultant_id; 

    // получение сообщений между пользователем и консультантом
